...
public class TeacherMyselfServiceImp implements ITeacherMyselfService {
    ...
    @Resource
    private XnCalendarMapper calendarMapper;//日历表
...
//        }
    ...
    public Map getMyTimeBook(int teacherId, String times) {
        ...
        //获取当月数据
        List<XnCalendarEntity> calendarEntities = this.calendarMapper.selectCalendarList(null, null, times);
        data_set = calendarEntities.stream().filter(c -> c.getHoliday()).map(XnCalendarEntity::getDataTime).collect(Collectors.toSet());
        data_set.addAll(getWeekendInMonth(Integer.parseInt(year), Integer.valueOf(month_)));
        data_set.removeAll(calendarEntities.stream().filter(c -> !c.getHoliday()).map(XnCalendarEntity::getDataTime).collect(Collectors.toSet()));
        //当月天数
        ...
        for (XnAbsentManage xnAbsentManage : list) {
            ...
            for (Date dataList : date) {
                ...
                if (times == null && "".equals(times)) {
                    if_time = new SimpleDateFormat("yyyy-MM").format(new Date());
                } else {
                    if_time = times;
                }
                if (if_time.equals(new SimpleDateFormat("yyyy-MM").format(dataList))) {
        ...
        data.put("times", times_.length() > 0 ? times_.substring(ConstantsDate.begin_1, times_.length() - 1) : "");//请假的具体时间
        data.put("time_num", time_num);//请假天数
        data.put("times_1", set_time);//请假时间
        data.put("time_rest", data_set.size());//休息天数
        data.put("num", maxDate - time_num - data_set.size());//出勤天数
        return data;
    }
     ...
     * @return
     */
    public static List getWeekendInMonth(Integer year, Integer month) {
        ...
        //判断第一天
        if (calendar.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY || calendar.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) {// 1代表周日，7代表周六 判断这是一个星期的第几天从而判断是否是周末
            list.add(year + "-" + (month < 10 ? "0" + month : month) + "-" +
                    (calendar.get(Calendar.DAY_OF_MONTH) < 10 ? "0" + calendar.get(Calendar.DAY_OF_MONTH) : calendar.get(Calendar.DAY_OF_MONTH)));// 得到当天是一个月的第几天
        }
