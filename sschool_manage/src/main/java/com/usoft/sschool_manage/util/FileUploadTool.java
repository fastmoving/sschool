...
public class FileUploadTool {
    ...
    public FileEntity createFile(HttpServletRequest request) {
        ...
        if (bflag) {
//            String logoPathDir = File.separator+"video"+File.separator;
//            String logoRealPathDir = request.getSession().getServletContext().getRealPath("");
            // 上传到本地磁盘
            // String logoRealPathDir = "E:/upload";
            String updatePath = fileBase + randomPackage();
//            File file = new File(logoRealPathDir);
            //创建新的文件夹
//            String updatePath = file.getParent()+logoPathDir;
            File logoSaveFile = new File(updatePath);
            ...
            // 绝对路径
            String fileNamedirs = updatePath + File.separator + newFileName + fileEnd;
            ...
            String fileDir = fileBase + newFileName + fileEnd;
...
//            boolean flag = false;
            if (this.checkAVIType(fileEnd)) {
                // 设置转换为AVI格式后文件的保存路径
                String codcAviPath = fileBase + File.separator + newFileName + ".avi";
            ...
            if (aviPath != null) {
                ...
                if (this.checkMediaType(fileEnd)) {
                    try {
                        // 设置转换为flv格式后文件的保存路径
                        String codcFilePath = fileBase + File.separator + newFileName + ".flv";
                        ...
                        fileDir = fileBase + newFileName + ".flv";
                ...
                entity.setSize(size);
                entity.setPath(randomPackage()+newFileName + fileEnd);
...
//     * @param fileName
    ...
    private String randomPackage(){
        ...
        String thisMonth = sdf1.format(today);
        ...
        String pa  = new String(File.separator+"video"+File.separator+thisMonth+File.separator+secondLevel+File.separator);
        return pa;
    }
}
